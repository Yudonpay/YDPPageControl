...
@IBDesignable open class YDPPageControl: UIView {
    // MARK: - PageControl
    ...
    fileprivate func layoutActivePageIndicator(_ progress: CGFloat) {
        ...
        let denormalizedProgress = progress * (indicatorDiameter + (indicatorPadding * 2))
        let distanceFromPage = abs(round(progress) - progress)
        let width = (indicatorDiameter
            + indicatorPadding * (distanceFromPage * 2)) * 2
    ...
    fileprivate func layoutInactivePageIndicators(_ layers: [CALayer]) {
        ...
        layers.forEach() { layer in
            ...
            layerFrame.origin.x += indicatorDiameter + (indicatorPadding * 2)
        }
    }
    ...
    override open func sizeThatFits(_ size: CGSize) -> CGSize {
        return CGSize(width: CGFloat(inactiveLayers.count) * indicatorDiameter + CGFloat(inactiveLayers.count - 1) * (indicatorPadding * 2),
                      height: indicatorDiameter)
    }
}
